...
export class Router {
	...
	constructor(Routes: Array<iRoute>, Prefixes: String[] = []) {
		this.Routes = Routes;
	 ...
	 */
	private SelectedRoute(Route: iRoute, Message: String[], Index: number) : Promise<iRoute> {
	 ...
	 */
	private GetDefaultChild(Route: iRoute) : Promise<iRoute> {
		return new Promise<iRoute> ((resolve, reject) => {
			Route.children.forEach((RouteChild) => {
