 ...
 */
function Pager(MessageRequest, Client, Embeds, Options = null) {
    ...
    MessageRequest.channel.send(Embeds[page - 1])
        ...
        if (!Options.allowallreactions) {
            var forwardreact = (reaction, user) => reaction.emoji.name === "➡" && user.id == MessageRequest.author.id && message.id == reaction.message.id;
            var backreact = (reaction, user) => reaction.emoji.name === "⬅" && user.id == MessageRequest.author.id && message.id == reaction.message.id;
        }
        else {
